...
{
    class PilotViewModel : ViewModelBase
    {
        ...
        public ICommand RegisterVelocityChangedObserver
        {
            get
            {
                if (_registerVelocityChangedObserver == null)
                {
                    ...
                    {
                        DJISDKManager.Instance.ComponentManager.GetFlightControllerHandler(0, 0).VelocityChanged += PilotHandingPage_VelocityChanged;
                    }, delegate () { return true; });
                }
                return _registerVelocityChangedObserver;
            }
        }
        private async void PilotHandingPage_VelocityChanged(object sender, Velocity3D? value)
        {
        ...
        public ICommand _moveAirCraft;
        public ICommand MoveAirCraft
        {
            get
            {
                if (_moveAirCraft == null)
                {
                    _moveAirCraft = new RelayCommand(async delegate ()
                    {
                        String message = "";
                        message = $"move aircraft, throttle: {Throttle}, roll: {Roll}, pitch: {Pitch}, yaw: {Yaw}";
